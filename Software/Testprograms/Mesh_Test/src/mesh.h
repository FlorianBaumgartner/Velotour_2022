...
class Mesh
{
  public:
    ...
    void setPayload(uint32_t data) {message[deviceId].payload = data;};
    int8_t getPersonalId(void) {return deviceId;};
    uint32_t getNodePayload(int8_t id) {return message[id % MAX_NODES_NUM].payload;};
    bool getNodeOrigin(int8_t id) {return origin[id % MAX_NODES_NUM] != 0;};
    uint8_t getNodeCount(void) {return nodeCount;};
